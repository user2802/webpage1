...
// if the "weight" variable in the input for sleep is given
if (isset($_POST['dates']) AND isset($_POST['numSteps']) AND isset($_SESSION['userId'])) {
    ...
    $query = <<<END
    INSERT INTO Steps(id, dates, steps)
    VALUES('{$_SESSION['userId']}','{$_POST['dates']}','{$_POST['numSteps']}')
    ON DUPLICATE KEY UPDATE steps = '{$_POST['numSteps']}';
END;
...
<div class=container>
    ...
    <div class="row">
        ...
        <form method="post" action="steps.php" id="formen">
            <h3 id="header">Steps metrics for a day</h3>
            <br>
            <label for="dateID" id="lab1">Choose date</label>
            <br>
            <input type="date" name="dates" id="dateID" required></input>
            <br>
            <label for="stepID" id="lab4">Number of steps</label>
            <br>
            <input type="number" name="numSteps" id="stepID" required></input>
            <br>
            <input type="hidden" name="distanceTravelled" id="walkID" value="walking" required></input>
            <br>
    ...
    <div class="row"  class="row" style="margin-top: 20px; margin-left: 30px;">
        ...
        <div class="alert alert-warning alert-dismissible fade hidden" role="alert" id="subAlert">
            ...
            <br>Number of steps have been successfully added.
